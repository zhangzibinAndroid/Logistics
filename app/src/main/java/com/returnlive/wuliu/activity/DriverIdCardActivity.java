 ...
 * 描述： 司机身份证照片页面
 */
public class DriverIdCardActivity extends AppCompatActivity {
    @ViewInject(R.id.tv_title)
    TextView tv_title;
    @ViewInject(R.id.img_drive_idCard_positive)
    ImageView img_drive_idCard_positive;
    @ViewInject(R.id.img_drive_idCard_reverse)
    ImageView img_drive_idCard_reverse;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_deiver_id_card);
        x.view().inject(this);
        tv_title.setText(getResources().getString(R.string.identity_certification));
        if (ConstantNumber.DRIVER_ACTION_PAGE==ConstantNumber.NUMBER_FIVE){
            img_drive_idCard_positive.setImageBitmap(DriverCertificationActivity.idCard_positive_bitmap);
            img_drive_idCard_reverse.setImageBitmap(DriverCertificationActivity.idCard_reverse_bitmap);
        }

    }

    @Event(value = {R.id.img_back, R.id.tv_save, R.id.img_drive_idCard_positive, R.id.img_drive_idCard_reverse})
    private void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.tv_save:
                ...
                idCard_positive_bitmap = ((BitmapDrawable)img_drive_idCard_positive.getDrawable()).getBitmap();
                idCard_reverse_bitmap = ((BitmapDrawable)img_drive_idCard_reverse.getDrawable()).getBitmap();
                ID_CARD_POSITIVE = 0;
                ID_CARD_REVERSE = 0;
                finishActivityWithData(ConstantNumber.NUMBER_FIVE);
                break;
            case R.id.img_drive_idCard_positive:
                whichIdCard = 1;
                showChoosePicDialog("身份证正面照");
                break;
            case R.id.img_drive_idCard_reverse:
                whichIdCard = 2;
            ...
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    ...
                    case TAKE_PICTURE: // 拍照
                        ...
                        String pathName = "";
                        if (whichIdCard==1){
                            pathName = "drive_idCard_positive.jpg";
                        }else if (whichIdCard==2){
                            pathName = "drive_idCard_reverse.jpg";
                        }
        ...
        if (resultCode == RESULT_OK) { // 如果返回码是可以用的
            switch (requestCode) {
                ...
                case CROP_SMALL_PICTURE:
                    if (data != null) {
                        if (whichIdCard==1){
                            setImageToView(data,img_drive_idCard_positive,"drive_idCard_positive"); // 让刚才选择裁剪得到的图片显示在界面上
                            DriverCertificationActivity.cardPath1 = imagePath;
                            ID_CARD_POSITIVE = 1;
                        }else if (whichIdCard==2){
                            setImageToView(data,img_drive_idCard_reverse,"drive_idCard_reverse");
                            DriverCertificationActivity.cardPath2 = imagePath;
                            ID_CARD_REVERSE = 1;
                        }
